cascade_size_time_nll <- function(par, model, data_clean, initator_responder, coefs, n_sims, fixed, n_time, time_tol, PRIVATE_ONLY, NULL_MODEL, fractional_contagion_subs) {
  
  time_tol <- time_tol

  model_result <- model(data_clean, initator_responder, par, coefs, n_sims, fixed, n_time, PRIVATE_ONLY, NULL_MODEL, fractional_contagion_subs)
...
social_private_model <- function(data_clean, initator_responder, params, coefs, n_sims, fixed, n_time, PRIVATE_ONLY, NULL_MODEL, fractional_contagion_subs) {
  ...
  for (i in unique(data$drop_ID)) {
    ...
    for (sim in 1:n_sims) {
      ...
      for (h in 1:length(drop_eel_IDs)) {
        ...
        resp_data[h,3] <- rbinom(n = 1, size = 1, prob = p_private_cue) #draw first responder
      ...
      if (sum(resp_data[,4], na.rm = TRUE) > 0) { #is there something weird happening that with no first responders, eels never get logged as suspectible?
        ...
        social_private_frame_recorder_matrix[fr_idx,1] <- 1 #they responded 
        
        for (fr_IDD in fr_idx) {
          state_matrix[fr_IDD,1] <- "i"  #those individuals are now infected
        }
        
        state_matrix[-fr_idx,1] <- "s" #everyone else is suspectible
        
        dosage_matrix[fr_idx,] <- NA #NA for dosage matrix for first responders
        dosage_matrix[-fr_idx,] <- 0 #0 for everyone else
        ...
        for (j in 1:length(drop_eel_IDs)) {
          ...
          if (state_matrix[j,k-1] == "r") { #if eel is recovered
            ...
            if (!is.na(frames_since_infected) && (frames_since_infected*dt >= tr)) {
              ...
              if ( PRIVATE_ONLY == FALSE) { #dose everyone
              
              for (jj in 1:length(drop_eel_IDs)) {
                if (jj == j || state_matrix[jj, k-1] != "s") next #if the to be dosed is yourself or they are are not suspectible, skip
                buddy_eel_ID <- drop_eel_IDs[jj] #assign the buddy eel / to be potentially dosed
                
                buddy_neighbours_ranked <- drop_data$inst_neighbours_topo_ranked[which(drop_data$colony_eel_ID == buddy_eel_ID)] #get the buddy eels ranking
                rank <- which(buddy_neighbours_ranked[[1]] == focal_eel_ID) #get the rank of the focal
                if (length(rank) == 0 || is.na(rank)) {
                  next 
                }
                log_inst_topo_dist_sc <- (log(rank) - orig_topo_mean) / orig_topo_sd #get the log rank scaled. 
                
                eta_j <- as.numeric(coefs[3]) + as.numeric(coefs[4])*log_inst_topo_dist_sc - social_decay_time_coef*frames_since_infected + as.numeric(coefs[5])*(drop_data$log_distance_to_ball_sc[jj]) - ball_decay_time_coef*k #calculate linear predictor from this rank

                w_ij <- 1/(1+exp(-eta_j))
                p_dose <- w_ij*max_rate*dt
                
                if (rbinom(1,1,p_dose) == 1) { #if the buddy does get a dose, their current doses state increases by 1
