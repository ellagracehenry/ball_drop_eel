...
coord_filename = "ball_drop_data_3D_trial17_ownint.xlsx" %ADD NAME OF BALL DROP FILE WITH 3D COORDS
imgcoordsRC_all = readtable(coord_filename);

total_final =[];
IDs = unique(imgcoordsRC_all{:,"drop_ID"});
for i = 1:length(unique(imgcoordsRC_all{:,"drop_ID"}))
    ...
    imgcoordsRC = imgcoordsRC_all(imgcoordsRC_all{:,3} == ID,:);
